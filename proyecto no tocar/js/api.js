...
// =============================================================================
// Modifica esta sección para cambiar la URL del servidor o los códigos.
// =============================================================================
...
(function () {
    ...
    // =========================================================================
    // 1. URL DEL SERVIDOR — Cambia esta línea si mueves el Google Apps Script
    // =========================================================================
    ...
    //    Agrega/quita objetos { code, coins, expiresAt } para gestionar códigos.
    // =========================================================================
    const _defaultCodes = [
        { code: atob("UExBWUEyMDI2"),     coins: 500,  expiresAt: "2026-12-31" },
        { code: atob("TUFSTEVSWTIwMjY="), coins: 1000, expiresAt: "2026-12-31" },
        { code: atob("U0VDUkVUQ09ERQ=="), coins: 1500, expiresAt: "2026-08-31" },
        { code: atob("QVJDQURFMjAyNg=="), coins: 300,  expiresAt: "2026-12-31" },
        { code: atob("Qk9OVVM1MDA="),     coins: 500,  expiresAt: "2026-10-31" },
        { code: atob("U1VQUkVNRTEwMA=="), coins: 100,  expiresAt: "2027-01-01" },
        { code: atob("R09MREVOQ09JTg=="), coins: 800,  expiresAt: "2026-11-30" },
        { code: atob("TEVWRUxVUA=="),     coins: 400,  expiresAt: "2026-09-30" },
        { code: atob("Q0hBTVBJT04="),     coins: 1200, expiresAt: "2026-07-31" },
        { code: atob("RlJFRUNPSU5T"),     coins: 200,  expiresAt: "2026-06-30" }
    ];

    // Cargar códigos personalizados guardados localmente
    ...
    window.saveToLeaderboard = function (playerName, score) {
        try {
            ...
            var streakNum = Number(window._gameMaxStreak) || 0;
            ...
            var wordFailsData = window._gameWordFailCount || {};
            ...
            window[callbackName] = function (response) {
                ...
                if (response && response.success) {
                    ...
                    // Guardar como pendiente si el servidor rechaza
    ...
    // Parche diferido: sincronizar perfil al cargar usuario y al guardar
    setTimeout(function () {
    ...
    //     Uso: En la consola, escribe: color = "#f0ca28"
    //     Luego puedes usar: score, coins, saveLeader, modLeader, addCode
    // =========================================================================
    var _adminUnlocked = false;
    var _adminLocked = true;
    var _storedColor = null;
    ...
    // Función para agregar códigos (solo admin)
    ...
    Object.defineProperty(window, 'color', {
        get: function () { return _storedColor; },
        set: function (v) {
            if (v === String.fromCharCode(35, 102, 48, 99, 97, 50, 56)) {
                _adminUnlocked = true; _adminLocked = false; _storedColor = v;
                _adminOk('Modo Admin desbloqueado. Comandos disponibles: score, coins, saveLeader, modLeader, addCode');
            } else {
                _adminUnlocked = false; _adminLocked = true; _storedColor = null; _adminDenied();
            }
        },
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Reintentar pendiente al cargar
        setTimeout(window.retrySendPending, 3000);
    });

    // Función auxiliar de escape HTML (disponible globalmente)
